...
Here’s a clean way to integrate the explanation of your `RunService.Heartbeat:Wait()` quirk at the end of the **Data & PlayerProfile** section. This explanation adds clarity on *why* it’s necessary without disrupting the flow of documentation:
...
This isn't just a random wait. It's a subtle quirk that **fixes a common issue with [`Profile.OnLastSave`](https://madstudioroblox.github.io/ProfileStore/api/#onlastsave)**, where the *shutdown reason* is incorrectly labeled as `"Manual"` instead of `"RobloxShutdown"` during proper game shutdown sequences.
